...
void ImageToSd()
{
 ...
 //DAMS - TCP connection
        Serial.println("Connect to TCP Receiver");
...
if (!tcpclient.connect(host, port)) {
        ...
        delay(1000);
          ...
          //Now read the .jpg with Lastfilenamevalue and tcpclient.write (not print!) the data!
           fs::FS &fs = SD_MMC;
          File tcpfile = fs.open(Lastfilenamevalue);

              // if the file is available, upload to TCP server:
        if (tcpfile) {
          
          while (tcpfile.available()) {
        
            tcpclient.write(tcpfile.read());

  
         }
      Serial.println("TCP Send completed for " + Lastfilenamevalue);
 
                
         tcpfile.close();
         const char* tussenchar = Lastfilenamevalue.c_str();
         deleteFile(SD_MMC,tussenchar );
        }  
