...
// 2019-June-15: "//JvZ" flags changes by Jim van Zee to allow an unmodified ESP32-CAM to be used, as well as one that
// has the "gpio33" HW mod described in the video link above.  The only difference in behavior is that the "FotoFlash"
// LED blinks when files are saved to the SD card since the LED is connected to the D1 data line.  Otherwise the Flash
// LED can be turned on or off at any time.  I also added some 'defines' to compile the program for your timezone.
...
const long  gmtOffset_sec = 360 * mijnTZ;
...
//  ImageToSd();//1st pic of the timelapse (//JvZ omit duplicate)
    tlrunningvalue  = true;
    resetTLcounter();
  }
}
...
//gpio_pulldown_dis(GPIO_NUM_4);    //JvZ re-config GPIO4
  ...
  if (!SD_MMC.begin()) {
    ...
    if (!fb) {
      ...
      return writeLED(LEDStatus); //JvZ restore calling state
    }
  ...
  writeLED(LEDStatus);  //JvZ restore calling state
}//end function
...
static void writeLED(bool LEDon)  //JvZ added GPIO4 on/off control
{
  LEDStatus = LEDon;
  pinMode(FOTO_FLASH, OUTPUT);    //JvZ config GPIO4 = LED driver
...
bool sdcardenable = true ; 
  ...
  writeLED(false);  //JvZ here after SD calls config SDMMC port (incl GPIO4)
